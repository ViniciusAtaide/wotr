---
source: GameFAQs WotR Guide 80843
game_patch: 2.7.0
---
# Bard Spells — Level 5
*17 spells. Source: GameFAQs WotR Guide (80843).*
*Format: **Name** *(School)* — T: target | Dur | Save | SR | Desc | Cast — Description. Fields omitted if empty/default.*

- **Animal Growth** *(Transmutation)* — _See Wizard spell list._ *(Beast Tamer only)*
- **Cacophonous Call, Mass** *(Enchantment)* — T: All enemies within 30' of target creature within close range | Dur: 1 round / level | Save: Will negates | SR | Desc: Mind-Affecting, Compulsion | Cast: Standard action — Affected enemies become nauseated for the spell's duration if they fail their Will saves.
- **Cloak of Dreams** *(Enchantment)* — _See Wizard spell list._
- **Cure Light Wounds, Mass** *(Conjuration)* — _See Cleric spell list._
- **Dispel Magic, Greater** *(Abjuration)* — _See Wizard spell list._
- **Heaven's Wing Strike** *(Evocation)* — T: All creatures within a 30' cone | Dur: 1d4 rounds | Save: Fortitude negates | SR | Desc: Good, Sonic | Cast: Standard action — Creatures in the area of effect must make a Fortitude save or become paralyzed for 1d4 rounds.
- **Heroism, Greater** *(Enchantment)* — _See Wizard spell list._
- **Joyful Rapture** *(Conjuration)* — _See Wizard spell list._
- **Ki Shout** *(Evocation)* — _See Wizard spell list._
- **Mind Fog** *(Enchantment)* — _See Wizard spell list._
- **Phantasmal Web** *(Illusion)* — _See Wizard spell list._
- **Resonating Word** *(Transmutation)* — _See Wizard spell list._
- **Shadow Evocation** *(Illusion)* — _See Wizard spell list._
- **Song of Discord** *(Enchantment)* — T: All enemies within 20' of target point within medium range | Dur: 1 round / level | Save: Will negates | SR | Desc: Mind-Affecting, Compulsion, Sonic | Cast: Standard action — Affected enemies have a 50% chance of attacking the nearest creature each round rather than acting normally.
- **Summon Large Elemental** *(Conjuration)* — _See Wizard spell list._
- **Summon Monster V** *(Conjuration)* — _See Wizard spell list._
- **Waves of Fatigue** *(Necromancy)* — _See Wizard spell list._ *(Dirge Bard only)*
