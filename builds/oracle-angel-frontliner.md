# Sword of the Battle Herald
**Oracle (Battle Mystery) — heavy-armour greatsword frontliner who doubles as the party's strongest divine caster via the Angel merged spellbook**

## At a Glance
| | |
| :--- | :--- |
| **Race** | Aasimar (Angelkin) — +2 Str, +2 Cha |
| **Attributes** | Str 18, Dex 12, Con 14, Int 10, Wis 10, Cha 16 (level-ups → Strength) |
| **Key class choices** | Curse: Lame · Mystery: Battle · Additional Spells: all "cure" spells |
| **Weapon & armour** | Greatsword + full plate (both from the Skill at Arms revelation) |
| **Mythic Path** | Angel — merged spellbook makes your caster level outrun a pure Cleric's |
| **Party Fit** | Beast Totem Skald in party — refuse Inspired Rage when you need to cast (it blocks your magic until Skald 20). Outflank partner: Regill, Greybor or Camellia (they take the feat too). This Oracle is the party's primary healer. |

## Levelling Table
| Level | Feat | Class Feature Choice | Notes |
| :--- | :--- | :--- | :--- |
| 1 | Power Attack | Revelation: **Skill at Arms** | Grants martial weapons + heavy armour |
| 3 | Toughness | Revelation: **Weapon Mastery** (Greatsword) | Free Weapon Focus now, Improved Critical at 8, Greater Weapon Focus at 12 |
| 5 | Cornugon Smash | — | Free demoralise on every Power Attack hit |
| 7 | Dazzling Display | Revelation: **War Sight** | Roll initiative twice; Uncanny Dodge |
| 9 | Shatter Defenses | — | Shaken enemies are flat-footed against you |
| 11 | Wings | Revelation: **Combat Healer** | Swift-action cures on yourself; Wings = +3 dodge AC vs melee, immune to ground effects |
| 13 | Armor Focus (Heavy) | — | |
| 15 | Improved Initiative | Revelation: **Iron Skin** | DR 10/adamantine, 2/day at this level |
| 17 | Combat Reflexes | — | |
| 19 | Outflank | Revelation: **Battlecry** | Partner: Regill, Greybor or Camellia — they must take Outflank too |
| 20 | — | **Final Revelation** (automatic) | Pounce; crits ignore DR; +4 AC vs crit confirmation |

**Curse milestones (Lame, automatic):** immune to fatigue at 5, armour never slows you at 10, immune to exhaustion at 15.

## Spell Selection
*Oracle is a spontaneous caster — these are your picks per spell level, in learning order. Battle Mystery and Additional Spells grant extras for free; never spend a pick on those.*

- **Level 1 (5 known):** Divine Favor, Shield of Faith, Bless, Remove Fear, Protection from Alignment
- **Level 2 (5 known):** Resist Energy, Restoration (Lesser), Aid, Effortless Armor, Grace
- **Level 3 (4 known):** Prayer, Dispel Magic, Searing Light, Resist Energy (Communal)
- **Level 4 (4 known):** Divine Power, Freedom of Movement, Death Ward, Protection from Energy (Communal)
- **Level 5 (4 known):** Breath of Life, Angelic Aspect, Flame Strike, True Seeing
- **Level 6 (3 known):** Heal, Eaglesoul, Blade Barrier
- **Level 7 (3 known):** Restoration (Greater), Holy Word, Resurrection
- **Level 8 (3 known):** Holy Aura, Frightful Aspect, Fire Storm
- **Level 9 (3 known):** Heal (Mass), Summon Monster IX, Energy Drain

**Free, don't pick:**
- *Battle Mystery bonus spells:* Enlarge Person (1), Bull's Strength (2), Magical Vestment (3), Enlarge Person Mass (4), Righteous Might (5), Bull's Strength Mass (6), Legendary Proportions (7), Rift of Ruin (8), Overwhelming Presence (9)
- *Additional Spells:* every Cure spell, all levels — this is what feeds Combat Healer
- *From mythic rank 3 onward:* the entire Angel spellbook (Sword of Heaven, Ward Against Harm Communal, etc.)

## Mythic Progression
| Rank | Ability / Feat Pick | Angel Path Choice |
| :--- | :--- | :--- |
| 1 | Enduring Spells | Close to the Heavens; Bypass Epic DR |
| 2 | Power Attack (Mythic) | Heaven's Breath |
| 3 | Greater Enduring Spells | Sword of Heaven; Angelic Halo; **merge spellbooks** |
| 4 | Weapon Focus (Mythic) | Improved Sword of Heaven: Everlasting Flame |
| 5 | Boundless Healing | Improved Angelic Halo: Flame of Life |
| 6 | Improved Critical (Mythic) | Improved Sword of Heaven: Grand Blessing |
| 7 | Mythic Charge | Halo's Holy Aura; Improved Angelic Halo: Solar Winds |
| 8 | Improved Initiative (Mythic) | Greater Sword of Heaven: Speed of Light; Wings |
| 9 | Thundering Blows | Improved Angelic Halo: Blinding Light |
| 10 | Spell Penetration (Mythic) | Greater Sword of Heaven: Overwhelming Flames; Sword of Holy Healing |

## How It Plays
**Levels 1–6:** a sturdy 3/4-BAB bruiser in full plate. Open fights with Divine Favor, lead with Power Attack, and patch yourself with cure spells between rounds. Lame's armour clause means full plate never slows you, and at 5 you stop caring about fatigue entirely.

**The spike — mythic rank 3:** merging spellbooks turns your caster level into class level + mythic rank, so you reach 6th-level spells around character 9 and 9th-level spells around 13. Sword of Heaven adds holy damage to your swings while Greater Enduring Spells stretches your buffs (Magical Vestment, Angelic Aspect, eventually Ward Against Harm Communal) to 24 hours — cast once in the morning, fight all day.

**Endgame loop:** Cornugon Smash shakes everything you Power Attack; Shatter Defenses makes the shaken flat-footed against you. Combat Healer means a Breath of Life on yourself is a swift action that never costs you a full attack. Speed of Light at rank 8 gives permanent Haste and quickens every spell of 7th level or lower. At 20, Pounce plus Mythic Charge turns every charge into a full attack with +10d6 divine damage riding on it. You finish as primary healer, best buff platform, and frontline damage dealer at once.

**Living with the Skald:** keep Inspired Rage toggled *off* on this character by default — accepting the song silences your spellcasting until the Skald reaches 20. You don't need the song's pounce anyway; your capstone provides it unconditionally. In fights where you have nothing to cast, flip the song on for the attack/damage bonus and flip it back when healing is needed.

## Variants & Trade-offs
- **Everlasting Flame doesn't combine with Greater Enduring Spells** — taken anyway; 1 min/rank covers any fight.
- **Heavenly Host skipped** — requires Bestow the Divine, which would cost both Improved Sword of Heaven picks.
- **Nature's Whispers (second-mystery Cha-to-AC) skipped** — it replaces your Dex bonus, which heavy armour already caps; right pick only for a Dex/robes variant.
- **Spell Resistance (L5 spell) cut for Angelic Aspect** — SR also blocks your own party's buffs in a pinch; Angelic Aspect's DR 5/evil suits a frontliner better.
- **Winds of Vengeance (L9) skipped** — its description is missing upstream; Energy Drain is a verified, stacking ranged-touch debuff instead.
- **The party Skald and the Pounce capstone:** Beast Totem song-pounce does *not* make Final Revelation redundant here — Inspired Rage blocks your spellcasting until the Skald reaches 20, so you'll refuse the song most fights and rely on your own Pounce. The capstone's crit-DR-bypass and +4 AC vs confirmation are never duplicated. At Skald 20, Master Skald's Haste overlaps Speed of Light — kept anyway for the ≤7th-level quicken.

---

# Appendix: Agent Verification

## A1. Data Integrity Checklist
- [x] **Literal String Match:** every named ability grepped verbatim in local files (rows below)
- [x] **Archetype Compatibility:** base Oracle used deliberately — every archetype trades Revelations and this build needs 6 (`classes/oracle.md:139`)
- [x] **Spell/Ability Origin:** Oracle casts from the Cleric list (`classes/oracle.md:11`; alias `spells/oracle/README.md`); all picks confirmed in `spells/cleric/`
- [x] **Prerequisite Chains:** all feat prereqs verified at level taken (rows below)
- [x] **Spells-Known Budget:** picks match the Oracle spells-known table 5/5/4/4/4/3/3/3/3 at 20 (`classes/oracle.md:66`)
- [x] **Stacking Audit:** see A4 — no same-type collisions
- [x] **Party-Composition Audit:** user confirmed party context (Beast Totem Skald; Outflank partner Regill/Greybor/Camellia; this Oracle is primary healer). Skald-vs-capstone interaction resolved — see Variants and rows below; Outflank partner named

## A2. Evidence Table
| Claim | Source | Status |
| :--- | :--- | :--- |
| Aasimar (Angelkin) +2 Str/+2 Cha, Celestial Resistance, 1/day Lesser Restoration | character-creation.md:222 | Verified |
| Level-up attribute points at 4/8/12/16/20 | character-creation.md:541 | Verified |
| Lame curse: −10' speed; fatigue immunity (5), armour speed clause (10), exhaustion immunity (15) | classes/oracle.md:109 | Verified |
| Battle Mystery bonus spell list (Enlarge Person → Overwhelming Presence) | classes/oracle.md:553 | Verified |
| Additional Spells: all cure spells added to spells known | classes/oracle.md:136–137 | Verified |
| Skill at Arms: martial weapon + heavy armour proficiency | classes/oracle.md:579 | Verified |
| Weapon Mastery: WF now, Imp. Critical @8, Greater WF @12 | classes/oracle.md:585 | Verified |
| War Sight: initiative twice, Uncanny Dodge @7, three rolls @11 | classes/oracle.md:582 | Verified |
| Combat Healer: swift-action cure (two slots), Oracle 7+ | classes/oracle.md:588 | Verified |
| Iron Skin: DR 10/adamantine, Oracle 11+ | classes/oracle.md:594 | Verified |
| Battlecry: +2 morale attack/skills/saves, 4/day @19 | classes/oracle.md:570 | Verified |
| Battle Final Revelation: Pounce, crits ignore DR, +4 AC vs confirmation | classes/oracle.md:600 | Verified |
| Power Attack (Str 13 ✓ at 1) | feats/weapon-feats.md:84 | Verified |
| Toughness | feats/miscellaneous-feats.md:68 | Verified |
| Cornugon Smash (Power Attack ✓, Persuasion 6 — class skill ✓) | feats/weapon-feats.md:105; classes/oracle.md:16 | Verified |
| Dazzling Display (Weapon Focus ✓ via Weapon Mastery) | feats/weapon-feats.md:49 | Verified |
| Shatter Defenses (WF ✓, Dazzling ✓, BAB 6 ✓ — Oracle +6 at 9; needs iteratives per guide caveat at :56) | feats/weapon-feats.md:53; classes/oracle.md:20 | Verified |
| Wings (Aasimar, level 10+) | feats/racial-feats.md:56 | Verified |
| Armor Focus (Heavy) | feats/armour-and-shield-feats.md:20 | Verified |
| Improved Initiative | feats/miscellaneous-feats.md:51 | Verified |
| Combat Reflexes | feats/miscellaneous-feats.md:19 | Verified |
| Outflank (teamwork); requires both flankers to have the feat ("you and an ally with this feat") | feats/teamwork-feats.md:16–18 | Verified |
| Oracle qualifies for Angel merge (Cleric/Druid/Oracle/Shaman); CL = class CL + rank; stat stays Cha | mythic/mythic-paths.md:70, 73–74 | Verified |
| Enduring Spells / Greater Enduring Spells | mythic/mythic-paths.md:158, 161 | Verified |
| Power Attack (Mythic) | mythic/mythic-paths.md:402 | Verified |
| Weapon Focus (Mythic) | mythic/mythic-paths.md:432 | Verified |
| Boundless Healing | mythic/mythic-paths.md:176 | Verified |
| Improved Critical (Mythic) | mythic/mythic-paths.md:384 | Verified |
| Mythic Charge (+1d6 divine/rank on charge) | mythic/mythic-paths.md:96 | Verified |
| Improved Initiative (Mythic) | mythic/mythic-paths.md:612 | Verified |
| Thundering Blows | mythic/mythic-paths.md:99 | Verified |
| Spell Penetration (Mythic) | mythic/mythic-paths.md:578 | Verified |
| Everlasting Flame (and its non-interaction with Greater Enduring Spells) | mythic/angel.md:55, 57 | Verified |
| Flame of Life | mythic/angel.md:78 | Verified |
| Grand Blessing | mythic/angel.md:58 | Verified |
| Solar Winds | mythic/angel.md:84 | Verified |
| Speed of Light (permanent Haste + quicken ≤7th) | mythic/angel.md:120 | Verified |
| Blinding Light | mythic/angel.md:72 | Verified |
| Overwhelming Flames | mythic/angel.md:117 | Verified |
| Heavenly Host requires Bestow the Divine | mythic/angel.md:112 | Verified |
| Nature's Whispers replaces Dex modifier to AC | classes/oracle.md:770 | Verified |
| L1 picks: Divine Favor / Shield of Faith / Bless / Remove Fear / Protection from Alignment | spells/cleric/cleric-l1.md:10, 20, 6, 18, 16 | Verified |
| L2 picks: Resist Energy / Restoration Lesser / Aid / Effortless Armor / Grace | spells/cleric/cleric-l2.md:28, 29, 5, 18, 20 | Verified |
| L3 picks: Prayer / Dispel Magic / Searing Light / Resist Energy Communal | spells/cleric/cleric-l3.md:15, 12, 21, 20 | Verified |
| L4 picks: Divine Power / Freedom of Movement / Death Ward / Protection from Energy Communal | spells/cleric/cleric-l4.md:10, 11, 8, 18 | Verified |
| L5 picks: Breath of Life / Angelic Aspect / Flame Strike / True Seeing | spells/cleric/cleric-l5.md:8, 5, 14, 29 | Verified |
| L6 picks: Heal / Eaglesoul / Blade Barrier | spells/cleric/cleric-l6.md:19, 16, 7 | Verified |
| L7 picks: Restoration Greater / Holy Word / Resurrection | spells/cleric/cleric-l7.md:18, 13, 19 | Verified |
| L8 picks: Holy Aura / Frightful Aspect / Fire Storm | spells/cleric/cleric-l8.md:13, 12, 11 | Verified |
| L9 picks: Heal Mass / Summon Monster IX / Energy Drain | spells/cleric/cleric-l9.md:6, 10, 5 | Verified |
| Winds of Vengeance — description missing upstream ("No Results Found") | spells/cleric/cleric-l9.md:12 | Unverified (upstream omission — excluded from picks) |
| Inspired Rage: "only the Skald herself can use magic" while affected | classes/skald.md:104 | Verified |
| Inspired Rage can be turned off per party member | classes/skald.md:110 | Verified |
| Skald shares pounce party-wide via Beast Totem rage power | classes/skald.md:137 | Verified |
| Master Skald (20): affected allies can cast spells + gain Haste | classes/skald.md:134–135 | Verified |

## A3. Source Map
See `wotr-guide.md` A3 for the full hub map. Files used by this build: `classes/oracle.md`, `classes/skald.md`, `mythic/mythic-paths.md`, `mythic/angel.md`, `feats/weapon-feats.md`, `feats/miscellaneous-feats.md`, `feats/racial-feats.md`, `feats/armour-and-shield-feats.md`, `feats/teamwork-feats.md`, `spells/cleric/cleric-l1..9.md`, `character-creation.md`.

Name variants encountered: "Flame Strike" is listed as "Flamestrike" in the guide's cleric summary table (annotated at `spells/cleric/cleric-l5.md:14`); "Restoration, Lesser/Greater" use the comma-inverted form in the spell files.

## A4. Stacking & Mechanics Notes
- **Divine Favor (luck)** vs **Divine Power (luck)** — same type, do NOT run both; Divine Power supersedes it from L4 on.
- **Battlecry (morale)** + **Bless/Aid/Prayer (morale)** — morale attack bonuses overlap; Battlecry's +2 covers them. Keep Prayer for the enemy −1 side.
- **Inspired Rage (Skald)** — attack/damage bonus has no stated type in the source (`classes/skald.md:104`); treat as potentially stacking but verify in-game. Moot in most fights since this character refuses the song to cast.
- Non-colliding stack: Magical Vestment (enhancement, armour) + Shield of Faith (deflection) + Wings (dodge) + Armor Focus (untyped) + Eaglesoul (sacred) + Frightful Aspect (size/natural) — all distinct types.
- Holy Aura (deflection +4) supersedes Shield of Faith when active — drop Shield of Faith from the L8+ rotation.
- Shatter Defenses requires iterative attacks (`feats/weapon-feats.md:56`) — satisfied from BAB +6 at character 9, exactly when the feat is taken.
