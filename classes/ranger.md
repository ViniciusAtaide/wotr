# Ranger
*Source: GameFAQs WotR Guide (80843).*

The Ranger has the following characteristics:

- Primary attributes: Dexterity and Wisdom
- Full AB progression
- May gain an Animal Companion at level 4
- Caster level progression from level 4
- May cast divine spells up to 4th-level
- Spells are prepared
- 7 HP per level
- High saving throws for Fortitude and Reflex, low saving throws for Will
- 4 skill points per level.

Class Skills: Athletics, Stealth, Knowledge (Arcana), Knowledge (World), Lore (Nature), Perception, Persuasion

BAB progression, saves and spells per day per level are shown in the table below:

| Lvl | AB | Fort | Reflex | Will | 1 | 2 | 3 | 4 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | +1 | +2 | +2 | +0 | - | - | - | - |
| 2 | +2 | +3 | +3 | +0 | - | - | - | - |
| 3 | +3 | +3 | +3 | +1 | - | - | - | - |
| 4 | +4 | +4 | +4 | +1 | 0 | - | - | - |
| 5 | +5 | +4 | +4 | +1 | 1 | - | - | - |
| 6 | +6 / +1 | +5 | +5 | +2 | 1 | - | - | - |
| 7 | +7 / +2 | +5 | +5 | +2 | 1 | 0 | - | - |
| 8 | +8 / +3 | +6 | +6 | +2 | 1 | 1 | - | - |
| 9 | +9 / +4 | +6 | +6 | +3 | 2 | 1 | - | - |
| 10 | +10 / +5 | +7 | +7 | +3 | 2 | 1 | 0 | - |
| 11 | +11 / +6 / +1 | +7 | +7 | +3 | 2 | 1 | 1 | - |
| 12 | +12 / +7 / +2 | +8 | +8 | +4 | 2 | 2 | 1 | - |
| 13 | +13 / +8 / +3 | +8 | +8 | +4 | 3 | 2 | 1 | 0 |
| 14 | +14 / +9 / +4 | +9 | +9 | +4 | 3 | 2 | 1 | 1 |
| 15 | +15 / +10 / +5 | +9 | +9 | +5 | 3 | 2 | 2 | 1 |
| 16 | +16 / +11 / +6 / +1 | +10 | +10 | +5 | 3 | 3 | 2 | 1 |
| 17 | +17 / +12 / +7 / +2 | +10 | +10 | +5 | 4 | 3 | 2 | 1 |
| 18 | +18 / +13 / +8 / +3 | +11 | +11 | +6 | 4 | 3 | 2 | 2 |
| 19 | +19 / +14 / +9 / +4 | +11 | +11 | +6 | 4 | 3 | 3 | 2 |
| 20 | +20 / +15 / +10 / +5 | +12 | +12 | +6 | 4 | 4 | 3 | 3 |

Where the number of spells per day shows 0, a Ranger may select a spell if his Wisdom is high enough to gain a bonus spell of that level.

The standard Ranger has the following level progression:

| Lvl | Feat | Special 1 | Special 2 |
| --- | --- | --- | --- |
| 0 | - | Ranger Proficiencies | Favored Enemy |
| 1 | Feat | - | - |
| 2 | - | Combat Style Feat | - |
| 3 | Feat | Endurance | Favored Terrain |
| 4 | - | - | Hunter's Bond |
| 5 | Feat | - | Favored Enemy |
| 6 | - | Combat Style Feat | - |
| 7 | Feat | - | - |
| 8 | - | - | Favored Terrain |
| 9 | Feat | Evasion | - |
| 10 | - | Combat Style Feat | Favored Enemy |
| 11 | Feat | - | Quarry |
| 12 | - | - | Camouflage |
| 13 | Feat | - | Favored Terrain |
| 14 | - | Combat Style Feat | - |
| 15 | Feat | - | Favored Enemy |
| 16 | - | Improved Evasion | - |
| 17 | Feat | - | - |
| 18 | - | Combat Style Feat | Favored Terrain |
| 19 | Feat | - | Improved Quarry |
| 20 | - | Master Hunter | Favored Enemy |

#### Ranger Proficiencies
Rangers are proficient with all simple and martial weapons, light and medium armour and shields.

#### Favored Enemy
The Ranger selects a creature type from the favoured enemies list and gains +2 bonus on attack and damage rolls against them. At 5th, 10th, 15th and 20th level, the Ranger selects an additional favoured enemy. At this point he may also select one of his favoured enemies and increase the bonus by another +2.

The categories that you can choose are: Aberrations (e.g. Oozes), Animals, Constructs (i.e. Golems), Demons of Magic, Demons of Slaughter, Demons of Strength, Dragons, Dwarves, Elves, Fey, Giant Humanoids, Gnomes, Goblins, Halflings, Humans, Magical Beasts (e.g. Hydra, Owlbear), Monstrous Humanoids (e.g. Centaurs), Outsiders (e.g. Elementals), Plants, Reptilian Humanoids, Undead and Vermin.

You will want to pick the various classes of demon as favoured enemies. Rather than trying to cover all three categories evenly, pick one category every time you select a favoured enemy so that you end up with +10 attack and damage against it. You can use the *Instant Enemy* spell to transfer this bonus to any enemy.

For your other slots, Constructs, Monstrous Humanoids and Undead are useful. For all intents and purposes, other enemy types either don't exist or are sufficiently rare that Instant Enemy can take care of them.

#### Combat Style Feat
The Ranger may select from a large variety of combat feats and does not need to meet the prerequisites to be able to select them. Besides the style feats, you can also select from Dodge, Great Fortitude, Iron Will, Lightning Reflexes and Toughness. From 10th level, you can select a Skill Focus.

##### Archery Feats
At 2nd level, the Ranger may choose from Point Blank Shot, Precise Shot and Rapid Shot. At 6th level, Improved Precise Shot, Point Blank Master and Manyshot are added. At 10th level, Improved Critical is added.

##### Menacing Feats
At 2nd level, the Ranger may choose from Dazzling Display, Power Attack and Intimidating Prowess. At 6th level, Cornugon Smash and Shatter Defenses are added. At 10th level, Dreadful Carnage and Improved Critical are added.

##### Two-Handed Weapon Feats
At 2nd level, the Ranger may choose from Cleave, Power Attack and Intimidating Prowess. At 6th level, Cleaving Finish and Great Cleave are added. At 10th level, Dreadful Carnage and Critical Focus are added.

##### Two-Weapon Combat Feats
At 2nd level, the Ranger can select from Double Slice, Shield Bash and Two-Weapon Fighting. At 6th level, Improved Two-Weapon Fighting is added. At 10th level, Greater Two-Weapon Fighting is added.

##### Weapon and Shield Feats
At 2nd level, the Ranger can select from Shield Bash, Shield Focus and Two-Weapon Fighting. At 6th level, Improved Two-Weapon Fighting and Shield Master are added. At 10th level, Bashing Finish and Greater Two-Weapon Fighting are added.

It is possible to take Greater Two-Weapon Fighting at level 10 without having taken Improved Two-Weapon Fighting at level 6. However, you only gain one off-hand attack rather than two.

#### Favored Terrain
At 3rd level, a Ranger may select a type of terrain and gains +2 bonus to initiative, Lore (Nature), Perception and Stealth skill checks when in that terrain. At 8th, 13th and 18th levels, the Ranger may select an additional terrain type. When doing so, the bonuses for one favoured terrain may be increased by +2. Choices are: Abyss, Desert, Forest, Highlands, Underground and Urban.

#### Hunter's Bond
The Ranger may bond with his companions. As a move action, he can grant half his favoured enemy bonus to all allies within 30' for a number of rounds equal to the Ranger's Wisdom modifier.

Alternatively, he may choose an Animal Companion. The companion shares the Ranger's favoured enemy and terrain bonuses. For companion capabilities, the Ranger's effective level is his class level - 3.

#### Evasion
At 9th level, if the Ranger makes a Reflex save against an attack that normally deals half damage on a successful save, he instead takes no damage.

#### Quarry
At 11th level, the Ranger can designate a target as his quarry as a standard action. He receives a +2 insight bonus to attack rolls made against the quarry and all critical hits are automatically confirmed. Only one creature at a time may be designated as a quarry.

#### Camouflage
At 12th level, the Ranger can use his Stealth skill to hide in any of his favoured terrains as a move action and may do so even in combat.

#### Improved Evasion
At 16th level, if the Ranger fails a Reflex save against an attack that normally deals half damage on a successful save, he takes only half damage.

#### Improved Quarry
At 19th level, the Ranger may designate a quarry as a free action and the insight bonus to attack rolls increases to +4.

#### Master Hunter
At 20th level, the Ranger can make an attack against a favoured enemy as a standard action at full bonus. If the attack hits, the target must make a Fortitude save (DC 20 + Wisdom modifier) or die. The Ranger can use this ability five times per day but not against the same creature.

##### Playing a Ranger
The Ranger is a very balanced class, gaining large bonuses against whole rafts of enemy types, and benefits from being played as a more-or-less pure class. I'd suggest choosing the Archery or Menacing combat styles. An Animal Companion is probably the better use of Hunter's Bond unless you're only taking a few Ranger levels for some reason. If you do choose an Animal Companion, take Boon Companion at 5th level.

However, there is no reason to play the base class because in *this* game, the Demonslayer is better in every way. Eventually, you will get +10 attack and damage against all three demon types. Other rangers could only get +6 vs. each category (or some other combination of 18). You get a better Quarry ability as well and even the Demonologist ability is better than Endurance. Best of all, these benefits cost nothing. A Demonslayer also makes an excellent 1-level dip.

Unfortunately, most other Ranger archetypes are not better than the base class and the Espionage Expert is a case in point, trading the very useful animal companion for a couple of somewhat useful skill check bonuses. At least an Espionage Expert keeps the somewhat useful Ranger's Bond ability which is more than you can say for the Flamewarden. I'm not quite sure what the point of the class is. Abilities that require you to *die* are not very good abilities.

A Freebooter can give the whole party decent bonuses against single powerful enemies. However, so can a Vanguard (Slayer) and Vanguard is the better class. The USP of a Ranger is favoured enemies and an animal companion and the Freebooter gets neither.

If you're planning on playing an archer anyway, a Nomad is strictly better than the base class, gaining a companion at 1st level and a bonus feat into the bargain. However, a horse is wasted if you're going to sit on it and shoot arrows rather than using it to charge down fools.

Similar to a Nomad, a Sable Company Marine gets a companion at 1st level and it is perhaps the best companion you can get. Flying Attack is - in effect - a teleport ability allowing you to pick your targets at will. You lose Favored Terrain but since that isn't very impactful, it's no great loss.

A Stormwalker gives up an animal companion for some questionable abilities. Being able to zip around the battlefield isn't *bad* in and of itself but on an archer, it's utterly pointless.

### Demonslayer
A Demonslayer does what it says on the tin.

- Gains exceptional abilities against demon enemies
- Cannot select other enemy types as favoured enemies.

He has the following level progression:

| Lvl | Feat | Special 1 | Special 2 |
| --- | --- | --- | --- |
| 0 | - | Ranger Proficiencies | Demonslayer Favored Enemy |
| 1 | Feat | - | - |
| 2 | - | Combat Style Feat | - |
| 3 | Feat | Demonologist | Favored Terrain |
| 4 | - | - | Hunter's Bond |
| 5 | Feat | - | Demonslayer Favored Enemy |
| 6 | - | Combat Style Feat | - |
| 7 | Feat | - | - |
| 8 | - | - | Favored Terrain |
| 9 | Feat | Evasion | - |
| 10 | - | Combat Style Feat | Demonslayer Favored Enemy |
| 11 | Feat | - | Fiendish Quarry |
| 12 | - | - | Camouflage |
| 13 | Feat | - | Favored Terrain |
| 14 | - | Combat Style Feat | - |
| 15 | Feat | - | Demonslayer Favored Enemy |
| 16 | - | Improved Evasion | - |
| 17 | Feat | - | - |
| 18 | - | Combat Style Feat | Favored Terrain |
| 19 | Feat | - | Improved Fiendish Quarry |
| 20 | - | Master Hunter | Demonslayer Favored Enemy |

#### Demonslayer Favored Enemy
A Demonslayer treats all three categories of demons as favoured enemies, gaining a +2 bonus to attack and damage at 1st level and an additional +2 at 5th, 10th, 15th and 20th levels.

#### Demonologist
At 3rd level, a Demonslayer can add half his class level to knowledge (Arcana) skill checks.

#### Expanded Spell List
A Demonslayer adds Protection from Evil as a 1st level spell and Protection from Evil (Communal) as a 2nd level spell.

#### Fiendish Quarry
At 11th level, when a Demonslayer chooses an evil outsider as a quarry, he may forego the +2 attack bonus and instead treat his weapons as good-aligned for the purpose of overcoming damage reduction.

#### Improved Fiendish Quarry
At 19th level, when a Demonslayer chooses an evil outsider as a quarry, he may forego the +4 attack bonus and grant his weapons the Holy property.

### Espionage Expert
An Espionage Expert is an urban take on the traditional Ranger.

- Gains skill bonuses
- Does not gain an animal companion
- Does not gain Favored Terrain bonuses.

He has the following level progression:

| Lvl | Feat | Special 1 | Special 2 |
| --- | --- | --- | --- |
| 0 | - | Ranger Proficiencies | Favored Enemy |
| 1 | Feat | Trapfinding | - |
| 2 | - | Combat Style Feat | - |
| 3 | Feat | Undercover | - |
| 4 | - | Espionage Expert Magic | Ranger's Bond |
| 5 | Feat | - | Favored Enemy |
| 6 | - | Combat Style Feat | - |
| 7 | Feat | - | - |
| 8 | - | - | - |
| 9 | Feat | Evasion | - |
| 10 | - | Combat Style Feat | Favored Enemy |
| 11 | Feat | - | Quarry |
| 12 | - | - | - |
| 13 | Feat | - | - |
| 14 | - | Combat Style Feat | - |
| 15 | Feat | - | Favored Enemy |
| 16 | - | Improved Evasion | - |
| 17 | Feat | - | - |
| 18 | - | Combat Style Feat | - |
| 19 | Feat | - | Improved Quarry |
| 20 | - | Espionage Master | Favored Enemy |

#### Trapfinding
An Espionage Expert adds half his class level to Perception checks.

#### Undercover
At 3rd level, an Espionage Expert adds half his class level to Bluff and Stealth checks.

#### Espionage Expert Magic
An Espionage Expert uses Charisma as his casting attribute.

#### Espionage Master
At 20th level, the Espionage Expert can make an attack against a favoured enemy as a standard action at full bonus. If the attack hits, the target must make a Fortitude save (DC 20 + Charisma modifier) or die. The Ranger can use this ability five times per day but not against the same creature.

### Flamewarden
A Flamewarden is a guardian of nature and an enemy of corruption in all its forms.

- May not select an Animal Companion
- Gains bonuses related to fire instead.

He has the following level progression:

| Lvl | Feat | Special 1 | Special 2 |
| --- | --- | --- | --- |
| 0 | - | Ranger Proficiencies | Favored Enemy |
| 1 | Feat | - | - |
| 2 | - | Combat Style Feat | - |
| 3 | Feat | Endurance | Favored Terrain |
| 4 | - | Spellcasting | Touch of Flame |
| 5 | Feat | - | Favored Enemy |
| 6 | - | Combat Style Feat | - |
| 7 | Feat | - | - |
| 8 | - | - | Favored Terrain |
| 9 | Feat | Stoking the Embers | Evasion |
| 10 | - | Combat Style Feat | Favored Enemy |
| 11 | Feat | - | Quarry |
| 12 | - | Burning Renewal | - |
| 13 | Feat | - | Favored Terrain |
| 14 | - | Combat Style Feat | - |
| 15 | Feat | - | Favored Enemy |
| 16 | - | Phoenix Blast | Improved Evasion |
| 17 | Feat | - | - |
| 18 | - | Combat Style Feat | Favored Terrain |
| 19 | Feat | - | Improved Quarry |
| 20 | - | Phoenix Rising | Favored Enemy |

#### Spellcasting
At 4th level the Flamewarden may select Firebelly and Summon Small Fire Elemental as 1st level spells. At 7th level, the Flamewarden may select Scorching Ray as a 2nd level spell. At 10th level, the Flamewarden may select Summon Medium Fire Elemental as a 3rd level spell. At 13th level, the Ranger may select Flamestrike, Summon Large Fire Elemental and Volcanic Storm as 4th level spells.

#### Touch of Flame
At 4th level, a Flamewarden can cause his weapons to burst into flame for 1 minute and can use the ability a number of times per day equal to 3 + his Wisdom modifier. Weapons so imbued count as magic for the purposes of overcoming damage reduction. At 12th level, weapons gain the *flaming burst* ability instead.

#### Stoking the Embers
At 9th level, the Flamewarden may restore life to a recently slain ally as a standard action once per day. If cast on an ally that has died within one round, the creature is brought back to life with 5d8 HP + 1 per caster level restored.

#### Burning Renewal
At 12th level, when the Flamewarden takes 10 or more points of fire damage, he removes the fatigued, exhausted, shaken, frightened, sickened and nauseated conditions. In addition, he restores 1d6 of ability damage and 1d6 points of ability drain from the most damaged attribute scores. This ability can be used a number of times per day equal to 3 + his Wisdom modifier. If the Flamewarden has fire resistance or immunity, the ability still triggers if he *would* have taken damage without the resistance.

#### Phoneix Blast
At 16h level, if a Flamewarden dies, his body explodes. Enemies within 10' take 10d6 damage (Reflex save for half) while allies are healed 5d6 HP and any poisons afflicting them are neutralized.

#### Phoenix Rising
At 20th level, the Flamewarden gains immunity to fire and once per day, if killed, returns to life with half his maximum HP.

### Freebooter
A Freebooter is the leader of a pirate crew.

- No favoured enemies
- May not select an Animal Companion
- Gains teamwork abilities instead.

He has the following level progression:

| Lvl | Feat | Special 1 | Special 2 |
| --- | --- | --- | --- |
| 0 | - | Ranger Proficiencies | Freebooter's Bane |
| 1 | Feat | - | - |
| 2 | - | Combat Style Feat | - |
| 3 | Feat | Endurance | Favored Terrain |
| 4 | - | - | Freebooter's Bond |
| 5 | Feat | - | Freebooter's Bane |
| 6 | - | Combat Style Feat | - |
| 7 | Feat | - | - |
| 8 | - | - | Favored Terrain |
| 9 | Feat | Evasion | - |
| 10 | - | Combat Style Feat | Freebooter's Bane |
| 11 | Feat | - | Quarry |
| 12 | - | - | Camouflage |
| 13 | Feat | - | Favored Terrain |
| 14 | - | Combat Style Feat | - |
| 15 | Feat | - | Freebooter's Bane |
| 16 | - | Improved Evasion | - |
| 17 | Feat | - | - |
| 18 | - | Combat Style Feat | Favored Terrain |
| 19 | Feat | - | Improved Quarry |
| 20 | - | Master Hunter | Freebooter's Bane |

#### Freebooter's Bane
The Freebooter can, as a move action, designate an enemy and have his allies focus on that target. The Freebooter and his allies gain +1 to attack and damage rolls against the designated target. This increases by +1 every five class levels (+5 at 20th level).

#### Freebooter's Bond
At 4th level, the Freebooter can expend a move action to grant allies within 30' an additional +2 bonus to attack rolls when flanking for 1 minute.

### Nomad
A Nomad is a mounted archery specialist.

- Gains a horse
- Can only select Archery style feats.

His level progression is as follows:

| Lvl | Feat | Special 1 | Special 2 |
| --- | --- | --- | --- |
| 0 | - | Ranger Proficiencies | Favored Enemy |
| 1 | Feat | Animal Companion | - |
| 2 | - | Archery Feat | - |
| 3 | Feat | Endurance | Favored Terrain |
| 4 | - | - | - |
| 5 | Feat | Point-Blank Master | Favored Enemy |
| 6 | - | Archery Feat | - |
| 7 | Feat | - | - |
| 8 | - | - | Favored Terrain |
| 9 | Feat | Evasion | - |
| 10 | - | Archery Feat | Favored Enemy |
| 11 | Feat | - | Quarry |
| 12 | - | - | Camouflage |
| 13 | Feat | - | Favored Terrain |
| 14 | - | Archery Feat | - |
| 15 | Feat | - | Favored Enemy |
| 16 | - | Improved Evasion | - |
| 17 | Feat | - | - |
| 18 | - | Archery Feat | Favored Terrain |
| 19 | Feat | - | Improved Quarry |
| 20 | - | Master Hunter | Favored Enemy |

#### Animal Companion
At 1st level, a Nomad receives a horse.

### Sable Company Marine
The Sable Company Marine archetype requires the *Dance of Masks* DLC.

A Sable Company Marine has trained to be able to handle and ride hippogriffs.

- Gains a unique mount
- Favored Enemy delayed
- Does not gain Favored Terrain bonuses.

His level progression is as follows:

| Lvl | Feat | Special 1 | Special 2 |
| --- | --- | --- | --- |
| 0 | - | Ranger Proficiencies | Hippogriff Companion |
| 1 | Feat | - | - |
| 2 | - | Combat Style Feat | - |
| 3 | Feat | The Strongest Wings | Favored Enemy |
| 4 | - | - | - |
| 5 | Feat | - | Favored Enemy |
| 6 | - | Combat Style Feat | - |
| 7 | Feat | - | - |
| 8 | - | - | - |
| 9 | Feat | Evasion | - |
| 10 | - | Combat Style Feat | Favored Enemy |
| 11 | Feat | - | Quarry |
| 12 | - | - | Feathered Confusion |
| 13 | Feat | - | - |
| 14 | - | Combat Style Feat | - |
| 15 | Feat | - | Favored Enemy |
| 16 | - | Improved Evasion | - |
| 17 | Feat | - | - |
| 18 | - | Combat Style Feat | - |
| 19 | Feat | - | Improved Quarry |
| 20 | - | Sable Strike | Favored Enemy |

#### Hippogriff Companion
A Sable Company Marine gains a hippogriff companion at 1st level. When mounted, the marine attacks the target of the hippogriff's flying attack as a full-round action. The target is considered flat-footed against these attacks and ranged attacks do not provoke attacks of opportunity.

#### The Strongest Wings
From 3rd level, as a standard action, a Sable Company Marine's companion can use its wings to attempt a bull rush combat manoeuvre against targets in a 30' cone. When doing so, it receives a bonus to CMB equal to half its level. Targets affected the combat manoeuvre are flat-footed against the next attack.

#### Feathered Confusion
From 12th level, a Sable Company Marine's companion receives a +3 dodge bonus to AC against melee attacks. While mounted, the marine receives this bonus as well. Enemies treat a 5' area around the hippogriff as difficult terrain.

#### Sable Strike
At 20th level, when using a flying attack against a favoured enemy, the first attacks made by the marine and his companion are automatic critical hits with +1 added to the critical multiplier.

### Stormwalker
A Stormwalker is a guardian of nature who can draw upon the power of storms.

- May not select an Animal Companion
- May not designate an enemy as a quarry
- May only select Archery feats as bonus feats.

He has the following level progression:

| Lvl | Feat | Special 1 | Special 2 |
| --- | --- | --- | --- |
| 0 | - | Ranger Proficiencies | Favored Enemy |
| 1 | Feat | - | - |
| 2 | - | Archery | - |
| 3 | Feat | Endurance | Favored Terrain |
| 4 | - | - | Wind Treader |
| 5 | Feat | - | Favored Enemy |
| 6 | - | Archery | - |
| 7 | Feat | - | - |
| 8 | - | - | Favored Terrain |
| 9 | Feat | Evasion | - |
| 10 | - | Archery | Favored Enemy |
| 11 | Feat | - | Flash Step |
| 12 | - | - | Camouflage |
| 13 | Feat | - | Favored Terrain |
| 14 | - | Archery | - |
| 15 | Feat | - | Favored Enemy |
| 16 | - | - | One with the Storm |
| 17 | Feat | - | - |
| 18 | - | Archery | Favored Terrain |
| 19 | Feat | - | Flash Shot |
| 20 | - | Master Hunter | Favored Enemy |

#### Archery
May select from the Ranger's list of Archery combat style feats.

#### Wind Treader
At 4th level, a Stormwalker can cause imbue his weapons with the *shock* ability for 1 minute and can use the ability a number of times per day equal to 3 + his Wisdom modifier. Weapons so imbued count as magic for the purposes of overcoming damage reduction. At 12th level, weapons gain the *shocking burst* ability instead.

#### Flash Step
At 11th level, the Stormwalker may move up to 50' without provoking attacks of opportunity as a full-round action. After doing so, he may make a single ranged attack against the nearest enemy.

#### One with the Storm
At 16th level, a Stormwalker gains immunity to electricity.

#### Flash Shot
At 19th level, the Stormwalker may make two ranged attacks at full attack bonus after using Flash Step.

### Ranger Spells

_Extracted to per-level files in `spells/ranger/` — see those for the full list._
